...
def venues():
    ...
    for venue in data:
        ...
        if len(areas) == 0:
            pos_area = 0
            area_item = {"city": venue.city, "state": venue.state, "venues": []}
        ...
        v = {"id": venue.id, "name": venue.name, "num_upcoming_shows": 4}
...
def search_venues():
    ...
    items = []
    for row in data:
        aux = {"id": row.id, "name": row.name, "num_upcoming_shows": len(row.shows)}
        items.append(aux)
